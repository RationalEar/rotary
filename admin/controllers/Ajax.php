...
class Ajax extends CI_Controller
{
	...
	{
		$file = $this->input->get('file');
		if( $this->input->get('fs') && $file )
		{
			if( unlink('./images/articles/main/'.$file) &&
				unlink('./images/articles/250x180/'.$file) &&
				unlink('./images/articles/100x75/'.$file) &&
				unlink('./images/articles/350x260/'.$file)
		...
		{
			$this->load->model('article_model');
			$data = $this->article_model->delete_product_image( array( 
			'im_file'=>$file, 'im_id'=>$this->input->get('id') ) );
			echo json_encode($data);
		}
	}
